...
public:
	...
	{
		int sent = send(m_client, mess, num_bytes, 0);
		if (sent == SOCKET_ERROR) {
			int err = WSAGetLastError();
			if (err != WSAEWOULDBLOCK) {
				// 실제 에러 — 메인 루프에서 disconnect 처리
			}
		}
	}
...
int main()
{
	...
	// server 소켓을 non-blocking으로 설정
	u_long mode = 1;
	ioctlsocket(server, FIONBIO, &mode);

	cout << "Pure Non-blocking I/O Server Started on port " << PORT << endl;

	for (;;) {
		// accept 폴링 — non-blocking이므로 연결이 없으면 INVALID_SOCKET 반환
		...
		// 모든 연결된 클라이언트를 순회하며 recv 폴링
		for (auto& cl : clients) {
			if (false == cl.m_is_connected) continue;
