		...
		if (move_packet->id < MAX_CLIENTS) {
			...
			if (ci == my_id) {
				if (0 != move_packet->move_time) {
					auto d_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count() - move_packet->move_time;
...
void Adjust_Number_Of_Client()
{
	...
	if (ACCEPT_DELY * delay_multiplier > duration_cast<std::chrono::milliseconds>(duration).count()) return;
	...
	if (DELAY_LIMIT2 < t_delay) {
		...
		if (ACCEPT_DELY * 10 > duration_cast<std::chrono::milliseconds>(duration).count()) return;
...
void InitializeNetwork()
{
	...
	test_thread = std::thread{ Test_Thread };
}
